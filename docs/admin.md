...
- **カバー画像** — エディタの「カバー画像」の枠に落とす。既定は 1200×630 で、記事の frontmatter の `heroImage` に入る
- **本文の画像** — 本文の入力欄に直接落とす。トリミングしたあと、カーソル位置に `![](/images/blogs/<slug>/<name>)` を差し込む
...
画像を入れるにはスラッグが決まっている必要がある（置き場所がスラッグで決まるため）。
新規作成でスラッグが空のうちは、その旨を出して受け取らない。
