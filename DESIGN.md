...
| A4 | **可読性優先の適用範囲** | タイプスケールは Tailwind 標準段（sm 0.875rem / base 1rem / lg 1.125rem …）。大文字マイクロラベルの適用は**ラベルのみ**（セクションアイブロウ、`dt`、メタストリップ）。日付・ステータス・説明文などのデータと本文は通常ケースで `sm` 以上、説明文は `base`。ステータスやタグは Badge（`rounded-badge` + 1px border）で表す。 | 13px 大文字がコンテンツ全域に及び可読性を損ねていたため。shadcn/ui の慣行（ラベルと本文の明確な分離、Badge によるステータス表示）に合わせる。 |
