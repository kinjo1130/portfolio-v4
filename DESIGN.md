...
**Inter 単一ファミリー**。中立で密度が高く、どこでも通用する。見出しと本文で書体を分けない（統一感を書体の共有で作る）。ベース 16px、**Tailwind 標準スケール**（A4 で 1.2 比から変更。可読性を優先し、shadcn/ui と同じ段を使う）。
...
| A4 | **可読性優先（shadcn 準拠）** | タイプスケールは Tailwind 標準段（sm 0.875rem / base 1rem / lg 1.125rem …）。**大文字マイクロラベルは全廃**（A3 を上書き）: ラベルは通常ケース `text-sm font-medium` + `text-secondary`。日付・ステータス・説明文などのデータと本文は `sm` 以上、説明文は `base`。ステータスやタグは Badge（`rounded-badge` + 1px border）。ページタイトルは `4xl`〜`5xl`（vw clamp の巨大見出しは廃止）。雑誌風の § メタストリップも廃止し、ヘッダーは border-b + ピル型ナビの構成。 | 13px 大文字がコンテンツ全域に及び可読性を損ねていたため。shadcn/ui の慣行に全面的に合わせる。 |
